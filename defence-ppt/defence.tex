...
% 	\title[Relation-aware Resource Provisioning and Management for Virtualized Services]{Relation-aware Resource Provisioning and Management for Virtualized Services}
	...
	\date[January 12th, 2018]{January 12th, 2018 \\ PhD Defence Presentation}
...
%     }
    ...
    \section{Introduction}
      \subsection{Computing-as-a-Service: The New Norm}
    \begin{frame}
%       \frametitle{* as a Service}
        \frametitle{Computing-as-a-Service: The New Norm}
         \begin{columns}
...
%           \path [decorate,decoration={text along path, text={Resource Abstraction}}] (3,-6.3) to[bend right] (7,-4);%(0.5,-6) -- (7,-4.5);
            ...
            \node[rectangle,color=blue] (c) at (0.7,-5) {Virtualization};
            \node[rectangle,color=blue] (c) at (0.7,-5.5) {Containers};
            \node[rectangle,color=blue] (c) at (0.5,-6) {Edge Computing};
            \node[rectangle,color=blue] (c) at (0.5,-6.5) {Serverless Computing};
            \begin{scope}
                \pgfsetlinewidth{2pt}
...
% Puru asked to change this a bit    
%    %%%%%%%%%%%% SLIDE %%%%%%%%%%%%%%    
%    \subsection{Virtualized Resources}
%    \begin{frame}
%	\frametitle{Resources Under Consideration}
%
%                \begin{figure}%[t]
...
%    \end{frame}
    ...
    \begin{frame}
        ...
        %\vspace{-1.4in}
        \begin{block}{1. Network-affinity aware CPU Usage Estimation}
	\textcolor{magenta}{Prediction of virtualized CPU usage for \emph{inter-PM} and \emph{intra-PM}
	...
	\frametitle{Thesis Contributions}
    	...
    	\begin{block}{2. VM Disk I/O Reduction by Host-cache Manipulation}
	\textcolor{magenta}{Reduction of disk I/O by exploiting content similarity within and across virtual machines}
	\vspace{0.2in}	
    ...
    \begin{frame}
        ...
        \begin{block}{Tools and Deliverables}
          \begin{enumerate}
    ...
    \subsection{Virtualized Resources}
    \begin{frame}
        \frametitle{Resources Under Consideration In Virtualized Environment}

                \begin{figure}%[t]
                \includegraphics[scale=0.55]{presyn-figures/resource-types.pdf} %\\
                \end{figure}
        ...
        \pgfsetlinewidth{1pt}
            \node[coordinate] (a) at (2.25, 5) {};
            \node[coordinate] (b) at (6.25, 5) {};
            \node[coordinate] (c) at (3.65, 0.45) {};
            \node[coordinate] (d) at (3.95, 2.1) {};

        \pgfsetlinewidth{2pt}
            \draw[-,style=dotted,color=purple] (a) -- (b);
            \draw[-,style=dotted,color=purple] (b) -- (c);
            \draw[-,style=dotted,color=purple] (c) -- (a);
        ...
        \pgfsetlinewidth{1pt}
            \node[coordinate] (p) at (10.25, 4.5) {};
            \node[coordinate] (q) at (7.5, 5.5) {};
            \node[coordinate] (r) at (5.7, 1.1) {};
            \node[coordinate] (s) at (8.55, 1.0) {};

        \pgfsetlinewidth{2pt}
            \draw[-,style=dotted,color=blue] (p) -- (q);
            \draw[-,style=dotted,color=blue] (q) -- (r);
            \draw[-,style=dotted,color=blue] (r) -- (s);
            \draw[-,style=dotted,color=blue] (s) -- (p);
	...
	\setbeamercolor{block title}{bg=darkpurple}
        \begin{block}{1. Network-affinity aware CPU Usage Estimation}
	Prediction of virtualized CPU usage for \emph{inter-PM} and \emph{intra-PM}
	network communication between VMs
        \end{block}

	\setbeamercolor{block title}{bg=title.bg}
        \begin{block}{2. VM Disk I/O Reduction by Host-cache Manipulation}
        Reduction of disk I/O by exploiting content similarity within and across virtual machines
        \end{block}
...
%         
    ...
    %%%%%%%%%%%%% SLIDE %%%%%%%%%%%%%%
    \begin{frame}
	\frametitle{Network-affinity aware CPU Usage Estimation}
	    \begin{block}{}
	    \begin{enumerate}
	     \item \textcolor{magenta}{Event profiling study} showing difference between \textit{intra-PM} 
		  and \textit{inter-PM} network code paths
	     \item \textcolor{magenta}{Benchmarking} of CPU usage
		  in colocated and dispersed scenarios for a VM pair	     
...
%%%%%%%%%%%% SLIDE %%%%%%%%%%%%%%
    ...
    \frametitle{Migration-Enabled Resource/Performance Management}
        \vspace{-0.15in}
        \begin{varblock}[11.0cm]{}
%       Migration for consolidation
        \textcolor{magenta}{Colocate} VMs for Resource Efficiency => \emph{intra-PM} network traffic
        \end{varblock}
        ...
        \vspace{-0.15in}
        \begin{varblock}[9.0cm]{}
%       Migration for QoS
        \textcolor{magenta}{Disperse} VMs for QoS => \emph{inter-PM} network traffic
        \end{varblock}
...
%           \item Our argument: Ignoring affinity might result in improper estimation
 ...
 \vspace{-0.2in}
\begin{tikzpicture}
\node[text height=2cm, text width=2cm, draw, rectangle] (a) at (9,15) {};
\node[text height=2cm, rectangle,color=blue] (b) at (9,15) {VM1};
\node[text height=2cm, rectangle] (c) at (6,15) {};
\node[text height=2cm, text width=2cm, draw, rectangle] (d) at (14,15) {};
\node[text height=2cm, rectangle,color=blue] (e) at (14,15) {VM2};
\node[text height=2cm, rectangle] (f) at (17,15) {};
\node[text height=2cm, color=purple, rectangle] at (9,16) {Web tier};
\node[text height=2cm, color=purple, rectangle] at (14,16) {Database tier};
\node[text height=2cm, color=blue, rectangle] (x) at (7,15) {n/w};
\node[text height=2cm, color=blue, rectangle] (x2) at (16,15) {n/w};
\node[text height=2cm, color=blue, rectangle] (y) at (7,14.5) {traffic};
\node[text height=2cm, color=blue, rectangle] (y2) at (16,14.5) {traffic};
\node[text height=2cm, color=blue, rectangle] (z) at (11.5,15) {traffic};
...
% \end{block}
\vspace{0.1in}
\begin{block}{Definition of Mutable n/w traffic}
 	For a VM pair, network traffic whose nature may 
    	\textcolor{magenta}{\textit{change between inter-PM and intra-PM}}
\end{block}
...
\vspace{0.1in}
    ...
    %%%%%%%%%%%%% SLIDE %%%%%%%%%%%%%%
    \subsection{Effect of colocation on CPU usage for Mutable N/w traffic}
    \frame{
        \frametitle{Effect of colocation on CPU usage for \textit{Mutable} N/w traffic}
...
%       Migration for QoS
        ...
        \begin{columns}
            ...
            (b) Dom0 CPU util for Rx/Tx %(summation versus co-located)
        \end{columns}
		...
		\item Linear correlation between network and CPU usage
             \end{itemize}
% \setbeamerfont{item}{size=\default}
    ...
    %%%%%%%%%%%%% SLIDE %%%%%%%%%%%%%%
    \subsection{Effect of colocation on CPU usage for Immutable N/w traffic, CPU and disk loads}
    \frame{
        \frametitle{Effect of colocation on CPU usage for Immutable n/w traffic, CPU and disk loads}

        \vspace{-0.1in}
        \begin{varblock}[12.0cm]{}
%       Migration for QoS
        \textcolor{black}{Benchmarking setup:} 4 VMs on 4 PMs---dispersed and colocated scenarios
...
\textbf{Rx (Mbps)} & \textbf{Dispersed case} & \textbf{Colocated case} \\
($<VM_1, VM_2>$) & $VM_1,VM_2, \sum Dom0_i$ & $VM_1,VM_2,Dom0$ \\ \hline
% $<$20, 10$>$ & 3, 2, 12 & 3, 2, ~8 \\ \hline
% $<$20, 30$>$ & 4, 5, 14 & 3, 5, 11 \\ \hline
$<$20, 50$>$ & 4, 7, \textcolor{darkpurple}{\textbf{18}} & 4, 7, \textcolor{darkpurple}{\textbf{14}} \\ \hline
% $<$20, 70$>$ & 4, 9, 21 & 4, 9, 17 \\ \hline
$<$40, 10$>$ & \textcolor{magenta}{6, 2}, 15 & \textcolor{magenta}{6, 2}, 11 \\ \hline
...
\end{table}
	...
	\begin{enumerate}
	 ...
	 \item Dom0 CPU usage change of \textcolor{darkpurple}{4\% for extra Dom0 instance (constant)}
	 \item \textcolor{black}{Similar observations for other workloads}---CPU and disk read/write
	\end{enumerate}
...
%           \node[coordinate] (n) at (1.5,2.25) {};
		...
		Build \textcolor{magenta}{linear prediction models} since correlation of CPU usage with all other resources usage is linear
        \end{exampleblock}
        ...
        \begin{block}{Parameters in the models}
         \begin{itemize}
          ...
          \item \textbf{Disk} metrics: read blocks/second, write blocks/second
           \item \textbf{Mutable and immutable} network metrics: Rx and Tx Kbps, packets/sec
         \end{itemize}
        \end{block}
...
%           \end{description}
        ...
        $CPU_{colocated} = f(\textcolor{blue}{CPU_1},\textcolor{blue}{Disk_1},\textcolor{blue}{Mutable_1},\textcolor{blue}{Immutable_1},$ 
	 \textcolor{white}{.}~~~~~~~~~~~~~~~~~~~~~~$\textcolor{blue}{CPU_2}, \textcolor{blue}{Disk_2}, \textcolor{blue}{Mutable_2}, \textcolor{blue}{Immutable_2})_{dispersed}$
        $CPU_{dispersed} = f(\textcolor{blue}{CPU_1},\textcolor{blue}{Disk_1},\textcolor{blue}{Mutable_1},\textcolor{blue}{Immutable_1})_{colocated}$ %, \textcolor{red}{CPU_2}, \textcolor{red}{Disk_2}, \textcolor{red}{Aff_2}, \textcolor{red}{NonAff_2})_{col}$
  ...
  \begin{frame}
    ...
    \vspace{-0.3in}
        \begin{columns}
            \begin{column}{0.5\textwidth}
	      \begin{figure}
		\includegraphics[scale=0.45]{defence-figures/combined_transition.pdf}
	      \end{figure}       
            \end{column}
            \begin{column}{0.4\textwidth}
		\begin{block}{Two-step prediction for combined transition}
		\begin{enumerate}
			\item Predict using \textit{dispersion model}
			\item Predict using \textit{colocation model} on previous prediction
		\end{enumerate}
		\end{block}

		\begin{block}{}
			Evaluated for multi-hop transitions
		\end{block}
            \end{column}
        \end{columns}
...
%        \item CPU requirement of communicating VMs can change upon VM migration.
        ...
        \begin{itemize}
            \item \textcolor{purple}{Synthetic and RUBiS workloads:} Max error within 1.5\% absolute CPU utilization for both DomU and Dom0 models
	...
	\begin{exampleblock}{Publications}
	\begin{itemize}
	\scriptsize
    		\item \textit{Affinity-aware Modeling of CPU Usage for Provisioning Virtualized Applications}. Proceedings of the 4th International Conference on Cloud Computing (CLOUD), 2011. Sujesha Sudevalayam and Purushottam Kulkarni.
    		\item \textit{Affinity-aware Modeling of CPU Usage with Communicating Virtual Machines}. Journal of Systems and Software (JSS), 2013. Sujesha Sudevalayam, Purushottam Kulkarni.
...
%       \subsection{Part II}
    \begin{frame}
	\frametitle{VM Disk I/O Reduction by Host-cache Manipulation}
	    \begin{block}{}
	      \begin{enumerate}
	      \item \textcolor{magenta}{Analysis of existing work (IODEDUP)} to 
				show inconsistent performance
	       \item \textcolor{magenta}{Redirection} of I/O requests from within VMs 
			and implicitly manipulate host-cache 
    ...
    \frame[shrink]{
        ...
        \begin{columns}
		  ...
		  \vspace{-0.1in}
		    ...
		    Similar operating systems, libraries, binaries, file copies
                \end{block}
...
%                     \scriptsize
                ...
                \begin{alertblock}{Two orthogonal solutions}
                    \begin{enumerate}
%                     \scriptsize
                    \item \textcolor{blue}{I/O deduplication} (IODEDUP\cite{iodedup}) : but \textcolor{red}{cache inclusiveness problem}
...
%   \subsection{Study of cache effectiveness for IODEDUP}
    ...
    \frametitle{Study of cache effectiveness for IODEDUP}
		...
		\textcolor{red}{Inconsistency} in achievable cache effectiveness
%            \scriptsize
...
%           the IODEDUP system is inconsistent.         
    ...
    \begin{frame}
        ...
        \begin{alertblock}{Issues}
            \begin{enumerate}
                \item IODEDUP system~\cite{iodedup} has cache inclusiveness problem
        ...
        \begin{itemize}
            ...
            that only data not present in cache will ever be fetched from disk
        \end{itemize}
        \end{block}
...
%   \subsection{System Requirements}
    ...
    \frametitle{DRIVE: Using implicit caching hints to achieve
        ...
        \underline{v}irtualized \underline{e}nvironments}
            ...
            \textbf{Our approach}: Augment the virtual disk driver to \textbf{\textcolor{magenta}{use implicit caching hints}}
	...
	\vspace{-0.2in}
	\begin{figure}
                    \includegraphics[scale=0.4]{defence-figures/drive-core-idea.pdf}
%                     \vspace{-0.25in}
                    \caption{I/O redirection based on content similarity}
                \end{figure}
	\end{frame}
    ...
    %%%%%%%%%%%%%%%%% SLIDE %%%%%%%%%%%%%%%%%%%%
    \begin{frame}
    \frametitle{Obtaining and using implicit hints for I/O redirection}
...
%                path for metadata update.
        ...
        \end{block}
         ...
         \only<2>{
             ...
             DRIVE: Redirects I/O requests to duplicate block addresses, to implicitly manipulate host-cache
             \end{exampleblock}
         }
    }
...
%           \caption{Split-driver architecture for virtual block devices.}
	...
	\vspace{-0.1in}
	\begin{block}{}
	    \textcolor{magenta}{To find other public datasets that can be used for our evaluation}
	\end{block}
...
% 	 \end{itemize}
	...
	\vspace{-0.40in}
		...
		\vspace{-0.15in}
		\caption{(a) Block access distribution}
	      \end{figure}
            \end{column}
            \begin{column}{0.5\textwidth}
	      \begin{figure}
		...
		\vspace{-0.15in}
		\caption{(c) Block reuse distribution}		
	      \end{figure}             
            \end{column}
	\end{columns}
	...
	\vspace{-0.25in}	
		...
		\vspace{-0.15in}
		\caption{(b) Duplicate block distrib}		
	      \end{figure}             
            \end{column}
            \begin{column}{0.5\textwidth}
	      \begin{figure}
		...
		\vspace{-0.15in}
		\caption{(d) Content reuse distribution}		
	      \end{figure}             
            \end{column}
	\end{columns}     
...
%             \scriptsize
            ...
            \item We concluded with a survey of publicly available datasets, and
		  benchmark generation literature, to make the case that
		  future work towards I/O deduplication benchmarks is necessary
        \end{itemize}
        \end{exampleblock}

	\begin{exampleblock}{Publication}	
	\scriptsize
	\textit{DRIVE: Using Implicit Caching Hints to achieve Disk I/O Reduction in Virtualized Environments}. Proceedings of the 21st International Conference on High Performance Computing (HiPC), 2014. Sujesha Sudevalayam, Purushottam Kulkarni, Rahul Balani and Akshat Verma.
	\end{exampleblock}
% 
%     \vspace{0.3in}
%     \begin{exampleblock}{}
