...
block-cache with a content-cache. Moreover, even in the implementation 
presented/evaluated in~\cite{iodedup} (refer Figure 5 in \cite{iodedup}
shown here as Fig.~\ref{fig:iodedup-arch}), 
the architecture includes both a block-cache as well as a content-cache. 
Hence, we have evaluated the same approach in our work.
